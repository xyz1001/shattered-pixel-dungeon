...
public class DictBook extends LocalItem {
    {
    ...
    @Override
    public String name() {
        return DictText.get("dictbook");
    }

    @Override
    public String desc() {
        return DictText.get("dictbook_d");
    }

    @Override
    public String actionName(String action, Hero hero) {
        if (action.equals(AC_READ)) {
            return DictText.get("ac_read");
        }
    ...
    public static class DictTab extends Component{
        ...
        private static class DictButton extends ListItem {
            ...
            public DictButton(String key, int imageSheet) {
                super(DictSpriteSheet.createImage(imageSheet), DictText.title(key));
                this.k = key;
            }

            public boolean onClick(float x, float y) {
                if (inside(x, y)) {
                    GameScene.show(new WndScrollTitledMessage(new Image(icon), DictText.title(k), DictText.get(k + "_d"), 152));
                    return true;
                }
                return false;
            }
